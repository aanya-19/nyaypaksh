...
def predict():
    ...
    dataset = data['dataset']  # Specifies the dataset type (e.g., 'tax', 'contract')

    # Check if dataset exists
    if dataset not in models:
        return jsonify({'error': f"Dataset '{dataset}' not found. Available datasets: {list(models.keys())}"}), 400

    # Preprocess the input text (case keywords, number of evidences, and court type)
    input_text = case_keywords + " " + number_of_evidences + " " + court_type
    processed_input = preprocess_input(input_text)
    
    # Vectorize the input text using the vectorizer for the chosen dataset
